...
sub main {
    ...
    while ($opt_engine_type = pop (@opt_engine_list)) {
        ...
        #print ("-------NO OF CALLS TO EXEC: ", $tests_completed, "\n");
...
sub parse_args {
    ...
    while (($option, $value) = nextOption()) {
        ...
        if ($option eq "b") {
        	...
        	&dd ("opt: call-graph output set to '$opt_bug_number'.");
        	$opt_bug_number = $value;
        }
          else {
            &usage;
        }
